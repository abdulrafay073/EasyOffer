...
class SalesAndMarketingController extends Controller
{
    ...
    public function saleMakeRequestSubmit(Request $request){
        try {
            ...
            for ($i = 0; $i < count($request->customername) ; $i++) {
                ...
                $buyer = Buyer::where('id', $request->customername[$i])->firstOrFail();
                // dd($buyer);

                $data = BuyerMakeRequest::create(
                    [
                        'buyer_id' => $request->customername[$i],
                ...
                $requestItems = BuyerMakeRequestDetail::create(
                    [
                        ...
                        'customer_name' => $buyer->comp_name_1,
    ...
    public function saleOrderSubmit(Request $request){
        try {
            ...
            //checkbox value
            if(isset($request->preshipmentcoa) ){ $preshipmentcoa = 1; } else { $preshipmentcoa = 0; }
            if(isset($request->shipmentafteradc) ){ $shipmentafteradc = 1; } else { $shipmentafteradc = 0; }
            if(isset($request->dml) ){ $dml = 1; } else { $dml = 0; }
            if(isset($request->preshipmentdocs) ){ $preshipmentdocs = 1; } else { $preshipmentdocs = 0; }
            if(isset($request->lables) ){ $lables = 1; } else { $lables = 0; }
            if(isset($request->gmp) ){ $gmp = 1; } else { $gmp = 0; }
            if(isset($request->certifictes) ){ $certifictes = 1; } else { $certifictes = 0; }
            if(isset($request->imagebeforeshipment) ){ $imagebeforeshipment = 1; } else { $imagebeforeshipment = 0; }
            if(isset($request->moa) ){ $moa = 1; } else { $moa = 0; }
            if(isset($request->preinformcharges) ){ $preinformcharges = 1; } else { $preinformcharges = 0; }
            if(isset($request->stability) ){ $stability = 1; } else { $stability = 0; }
            if(isset($request->safta) ){ $safta = 1; } else { $safta = 0; }
            if(isset($request->materialavailability) ){ $materialavailability = 1; } else { $materialavailability = 0; }
            if(isset($request->dmf) ){ $dmf = 1; } else { $dmf = 0; }
